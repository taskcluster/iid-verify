...
napi_value Call_VF_verify(napi_env env, napi_callback_info info) {
  ...
  if (status != napi_ok) {
      napi_throw_error(env, NULL, "failed to get information about pubkey buffer");
  }
  status = napi_get_buffer_info(env, argv[2], &document, (size_t*)&document_l);
  if (status != napi_ok) {
      napi_throw_error(env, NULL, "failed to get information about document buffer");
  }
  status = napi_get_buffer_info(env, argv[4], &signature, (size_t*)&signature_l);
  if (status != napi_ok) {
      napi_throw_error(env, NULL, "failed to get information about signature buffer");
  }

  return_t result;
  result = VF_verify(pubkey, pubkey_l, document, document_l, signature, signature_l);
