...
return_t VF_verify(char* pubkey,         uint64_t pubkey_length,
  ...
  //BIO_set_close(bio_pkcs7_envelope, BIO_NOCLOSE);
  ...
  //
  //   if(!PEM_read_bio_PKCS7(bio_pkcs7_envelope, &p7, NULL, NULL)) {
  //
  p7 = PEM_read_bio_PKCS7(bio_pkcs7_envelope, NULL, NULL, NULL);
  if (p7 == NULL) {
    rv = VF_FAIL;
    goto end;
  }
  ...
  if (!BIO_free(bio_document) || !BIO_free(bio_pubkey) || !BIO_free(bio_pkcs7_envelope)) {
    rv = VF_FAIL;
  }
